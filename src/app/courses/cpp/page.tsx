...
export default function CppCourses() {
  const [cppLessons, setCppLessons] = useState<Lesson[]>([]);
  ...
  useEffect(() => {
    const loadLessonsAndProgress = async () => {
      const cppLessons = lessons.map((lesson: Lesson) => ({
        ...lesson,
        courseId: 'cpp'
      }));
      setCppLessons(cppLessons);
      if (cppLessons.length > 0) setSelectedLesson(cppLessons[0]);

      if (userId) {
        try {
          const response = await fetch(`/api/progress?userId=${userId}&courseId=cpp`);
  ...
  const progressPercentage = cppLessons.length > 0
    ? (Object.values(progress).filter(Boolean).length / cppLessons.length) * 100
    : 0;
                ...
                if (isConfirmed) {
                  try {
                    ...
                    const response = await fetch('/api/progress/reset', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ userId, courseId: 'cpp' })
                    });
